...
import static org.libsodium.rn.ImagePickerModule.*;
...
public class Utils {
    ...
    static byte[] getBytes(Uri uri, ReactContext reactContext) {
          InputStream inputStream;
          try {
              inputStream = reactContext.getContentResolver().openInputStream(uri);
          } catch (FileNotFoundException e) {
    ...
    static ReadableMap getImageResponseMap(Uri uri, Options options,ReadableMap rawOptions, ReactContext context) {
        ...
        byte[] bytes = getBytes(uri,context);
    ...
    static ReadableMap getResponseMap(List<Uri> fileUris, Options options,ReadableMap rawOptions, ReactContext context) throws RuntimeException {
        ...
        for(int i = 0; i < fileUris.size(); ++i) {
            ...
            if (isImageType(uri, context)) {
                ...
                assets.pushMap(getImageResponseMap(uri, options,rawOptions, context));
